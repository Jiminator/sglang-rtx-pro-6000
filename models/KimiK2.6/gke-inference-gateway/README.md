...
This repository contains the configuration files and test scripts to set up and verify **Precise Prefix-Cache Aware Routing** using GKE Inference Gateway and vLLM.

This setup ensures that requests sharing the same prompt prefix are routed to the same vLLM replica, maximizing KV cache reuse and reducing Time-to-First-Token (TTFT).
